...
public class SeqlockReaderBenchmark {
    ...
    private static void pinIfRequested(int cpu) {
        ...
        if (LOG_PIN) {
            ...
            System.out.printf(
                    Locale.ROOT,
                    "[pin] role=reader requestedCpu=%d actualCpu=%d status=%s thread=%s%n",
                    cpu,
                    actualCpu,
                    status,
    ...
    @State(Scope.Thread)
    public abstract static class ReaderState {
        @Param({"256"})
        public int numSlots;
        ...
        private final Snapshot64 seedSnapshot = new Snapshot64();
        ...
        public void setup() {
            ...
            fillSnapshot(seedSnapshot, FIXED_ID, 1);
            seedSnapshot.long0 = System.nanoTime();
            table.write(FIXED_ID, seedSnapshot);
        ...
        long readOne() {
            table.tryLoad(FIXED_ID, readerSnapshot);
        ...
        public void tearDownIteration() {
            ...
            System.out.printf(
                    Locale.ROOT,
                    "[age] impl=%s mode=reader-only numSlots=%d fixedId=%d p50=%dns p99=%dns p99.9=%dns samples=%d%n",
                    implName(),
                    numSlots,
                    FIXED_ID,
                    p50,
    ...
    public static class OffHeapState extends ReaderState {
        @Override
    ...
    public static class HeapNaiveState extends ReaderState {
        @Override
        protected String implName() {
            return "heap-naive";
        }
    ...
    public static class HeapAlignedState extends ReaderState {
        @Override
        protected String implName() {
            return "heap-aligned";
        }
    ...
    @Benchmark
    public long offheapReaderOnly(OffHeapState s) {
        pinIfRequested(READER_CPU);
        return s.readOne();
    }

    @Benchmark
    public long heapNaiveReaderOnly(HeapNaiveState s) {
        pinIfRequested(READER_CPU);
        return s.readOne();
    }

    @Benchmark
    public long heapAlignedReaderOnly(HeapAlignedState s) {
        pinIfRequested(READER_CPU);
        return s.readOne();
    }
